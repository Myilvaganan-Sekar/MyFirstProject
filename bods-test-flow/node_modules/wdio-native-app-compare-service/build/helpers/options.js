...
exports.defaultCompareOptions = defaultCompareOptions;
exports.initOptions = initOptions;
exports.instanceCompareOptions = instanceCompareOptions;
...
function initOptions(options) {
  ...
  if (!('baselineFolder' in newOptions)) {
    const baselineFolderMessage = `'baselineFolder'-property is missing in the wdio-config file! 'baselineFolder' set to '${_constants.DEFAULT_BASELINE_FOLDER}'`;

    _logger.nativeAppCompareLog.warn(baselineFolderMessage);

    init.baselineFolder = _constants.DEFAULT_BASELINE_FOLDER;
  }

  if (!('screenshotPath' in newOptions)) {
    const screenshotsFolderMessage = `'screenshotPath'-property is missing in the wdio-config file! 'screenshotPath' set to '${_constants.DEFAULT_SCREENSHOTS_FOLDER}'`;

    _logger.nativeAppCompareLog.warn(screenshotsFolderMessage);

    init.screenshotPath = _constants.DEFAULT_SCREENSHOTS_FOLDER;
  }
 ...
 *   work on Android
 * @param {boolean} options.blockOutIphoneHomeBar Block out the iPhoneX bottom bar
...
function instanceCompareOptions(options) {
  return { ...(options.blockOuts ? {
      blockOuts: options.blockOuts
    } : {}),
    ...(options.blockOutStatusBar ? {
      blockOutStatusBar: options.blockOutStatusBar
    } : {}),
    ...(options.blockOutIphoneHomeBar ? {
      blockOutIphoneHomeBar: options.blockOutIphoneHomeBar
    } : {}),
    ...(options.blockOutNavigationBar ? {
      blockOutNavigationBar: options.blockOutNavigationBar
    } : {}),
    ...(options.elementBlockOuts ? {
